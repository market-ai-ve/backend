...
describe('TestCas entity buyer person', () => {
    ...
    test('Should return class with passed success params', () => {
        ...
        const buyerPerson = new BuyerPerson(companyName, companyDescription, dataSearch)

        expect(buyerPerson.toString()).toEqual(`company name: ${companyName} \n\ncompany description: ${companyDescription} \n\n data search: ${dataSearch.join(', ')}`)
    })
    ...
    test('Should return error when passing the dataSearch parameter as data other than Array', () => {
        ...
        expect(() => {
            // @ts-ignore
            return new BuyerPerson(companyName, companyDescription, dataSearch).toString()
        }).toThrow(TypeError)
        expect(() => {
            // @ts-ignore
            return new BuyerPerson(companyName, companyDescription, dataSearch).toString()
